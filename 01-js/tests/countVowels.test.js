...
describe('countVowels', () => {
    test('returns the correct count for strings with vowels', () => {
        ...
        expect(countVowels('programming')).toBe(3); // corrected from 4
        expect(countVowels('OpenAI')).toBe(4); // corrected from 3
    });

    test('returns 0 for strings without vowels', () => {
        expect(countVowels('rhythm')).toBe(0);
        expect(countVowels('fly')).toBe(0);
        expect(countVowels('lymph')).toBe(0); // changed from chatbot which has vowels
    });
    ...
    test('handles case-insensitivity correctly', () => {
        expect(countVowels('EaSiEr')).toBe(4); //changed from 3
