...
router.post('/signin', async (req, res) => {
  ...
  if (!userExists) res.status(404).json({ message: "UNAUTHORISED: user does not exist" })
  if (!(bcrypt.compare(password, userExists.password))) return res.status(401).json({ message: "wrong password" })
